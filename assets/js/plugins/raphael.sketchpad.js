...
(function(Raphael) {
	...
	var SketchPad = function(paper, options) {
		...
		function is_mobile_tablet_user_agent() {
			// iPhone Events
			var agent = navigator.userAgent;
			return agent.indexOf("iPhone") > 0 || agent.indexOf("iPod") > 0
				|| agent.indexOf("iPad") > 0 || agent.indexOf("Android") > 0
		...
		self.editing = function(mode) {
			...
			if (_options.editing) {
				if (_options.editing == "erase") {
					...
					if (is_mobile_tablet_user_agent()) {
